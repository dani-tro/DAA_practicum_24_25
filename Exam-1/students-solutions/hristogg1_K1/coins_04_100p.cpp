...
bool isItPossible(int dist) {
    ...
    for (int i = 0; i < m; ++i) {
        for (int j = logs[i].first; j <= logs[i].second;) {
            if (j >= lastPutPos + dist) {
                lastPutPos = j;
                put++;
                j += dist;
            }
            else if (lastPutPos + dist > logs[i].first && lastPutPos + dist <= logs[i].second)
                j = lastPutPos + dist;
            else
                break; 
        }
...
int main() {
    ...
    unsigned long long l = 1, r = 1 + ((logs[m - 1].second) + 1 / n);
    while (l < r) {
